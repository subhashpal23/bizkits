...
*/
class Tax extends Common_Controller 
{
	private $moduleName;
	private $user_id;
	private $role;
	...
	{	  	 
		...
		$data['order_type']='tax';
		$data['module_name']=$this->moduleName;
		$data['callback_name']=$this;
		_adminLayout("ecommerce/order-mgmt/all-orders-tax",$data);
	}
	...
	public function getProductTax($id,$type,$order_id)
	{
	    //echo $id.','.$type;
	    if($type=='STOCKIST')
	    {
	        $result1=$this->db->query("SELECT stockist_id from eshop_stockist_sell where order_id='".$order_id."' order by id desc")->row();
	        $stockist_id=$result1->stockist_id;
	        $result=$this->db->query("SELECT taxper from eshop_stock_stockist where product_id='".$id."' and stockist_id='".$stockist_id."' order by id desc")->row();
	        return $result->taxper?$result->taxper:'0';
	    }
	    else
	    {
	        $result=$this->db->query("SELECT taxper from eshop_products where id='".$id."' order by id desc")->row();
	        //pr($result);
		    return $result->taxper?$result->taxper:'0';
	    }
