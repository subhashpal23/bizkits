...
class Member extends Common_Controller 
{
	...
	{
	    $data['allcommission']=$all_members=$this->member_model->getDirectCommission();
	...
	public function viewAllMember($type=false)
	{
        ...
        $totalRec=$all_members=$this->member_model->getAllMembers($conditions);
        //echo $totalRec;exit;
		...
		$data['all_members']=$all_members=$this->member_model->getAllMembers($conditions);
	...
	public function viewPromoMember($type=false)
	{
        ...
        //pr($conditions);
        //$totalRec = $this->post->getRows($conditions); 
        $totalRec=$all_members=$this->member_model->getPromoMembers($conditions);
        //echo $totalRec;exit;
        // Pagination configuration 
		...
		//$data['type']=$type;
		$data['conditions']=$conditions;
		$data['all_members']=$all_members=$this->member_model->getPromoMembers($conditions);
		//echo "<pre>";print_r($all_members);
	...
	public function activeMember($type=false)
	{
		...
		$data['all_members']=$all_members=$this->member_model->getAllActiveMembers();
	...
	public function inactiveMember($type=false)
	{
		...
		$data['all_members']=$all_members=$this->member_model->getAllInActiveMembers();
		_adminLayout("member-mgmt/active-member",$data);
	...
	public function blockUnblockMember()
	{
		...
		$totalRec=$all_members=$this->member_model->getAllBlockUnBlockMembers($conditions);
		$config['base_url']    = base_url().'Admin/Member/viewAllMember/'; 
        $config['uri_segment'] = $uriSegment; 
        $config['total_rows']  = $totalRec; 
        $config['per_page']    = $this->perPage; 
		...
		//$data['type']=$type;
		$data['conditions']=$conditions;
		$data['all_members']=$all_members=$this->member_model->getAllBlockUnBlockMembers($conditions);
		_adminLayout("member-mgmt/active-member",$data);
	...
	public function passwordTracker()
	{
		$data=array();
		$data['title']='Block/Unblock Members';
		//$data['all_members']=$all_members=$this->member_model->getAllMembersPassword();
		$data = $conditions = array(); 
        $uriSegment = 4; 
        ...
        //$totalRec = $this->post->getRows($conditions); 
        $totalRec=$all_members=$this->member_model->getAllMembersPassword($conditions);
        ...
        $data['conditions']=$conditions;
		$data['all_members']=$all_members=$this->member_model->getAllMembersPassword($conditions);
	...
	public function resetTPassword($user_id=null)
	{
		...
		_adminLayout("member-mgmt/reset-tpassword",$data);	
	}
}//end class