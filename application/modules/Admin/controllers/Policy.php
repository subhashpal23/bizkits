...
class Policy extends Common_Controller 
{
	...
	public function editDashboardNotice()
	{
	    if(!empty($this->input->post('btn')))
		{
		    ...
		    if($profile_pic!='')
		    {
		        $this->db->update('confidential',
		 	               array('confidential_value'=>$privacy_policy,'image'=>$profile_pic,'status'=>$status,'last_updated'=>date('Y-m-d H:i:s')),
		 	               array('confidential_key'=>'notice')
		 	               );
		    }
		    else
		    {
		        $this->db->update('confidential',
		 	               array('confidential_value'=>$privacy_policy,'status'=>$status,'last_updated'=>date('Y-m-d H:i:s')),
		 	               array('confidential_key'=>'notice')
		 	               );
		    }
		    $this->session->set_flashdata("flash_msg", '<span class="text-semibold">Well done!</span> Notice is updated successfully');
            redirect(ci_site_url().'Admin/Policy/editDashboardNotice');
		}
		$data=array();
		$data['privacy_policy']=$this->policy_model->getNotice();
		$data['privacy_policy1']=$this->policy_model->getNotice1();
	 	_adminLayout("policy-mgmt/edit-notice",$data);
	}
	public function editDashboardNotice1()
	{
	    if(!empty($this->input->post('btn')))
		{
		    $privacy_policy=$this->input->post('privacy_policy');
		    ...
		    if($profile_pic!='')
		    {
		        $this->db->update('confidential',
		 	               array('confidential_value'=>$privacy_policy,'image'=>$profile_pic,'status'=>$status,'last_updated'=>date('Y-m-d H:i:s')),
		 	               array('confidential_key'=>'notice1')
		 	               );
		    }
		    else
		    {
