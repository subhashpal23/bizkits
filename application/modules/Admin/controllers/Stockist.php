...
class Stockist extends Common_Controller 
{
	...
	public function allStockist()
	{
		 ...
		 $result=$this->db->query("SELECT * FROM user_registration where member_type='2' order by id")->result_array();	     	
		 $data['all_category']=$result;
		 $data['module_name']=$this->moduleName;
		 $data['controller_name']=$this->controllerName;
	...
	public function showStock($del_id)
	{
	    ...
	    $adminsub=$this->db->query("SELECT * from user_registration where member_type='2' and id='".$delid."'  order by id desc")->row();
		$result=$this->db->query("SELECT * from eshop_products where status='1'  order by id desc")->result_array();	     	
		$data['all_products']=$result;
		$data['module_name']=$this->moduleName;
		$data['stockist_detail']=$adminsub;
		$data['controller_name']=$this;
		//$data['stockist_id']=$delid;
		$data['stockist_id']=$adminsub->user_id;
	...
	public function setAjaxStockistAll()
	{
	    ...
	    $remove_stockist=$this->input->post('remove_stockist');
	    ...
	    $qtyall=0;
	    if($type=='remove_stockist_')
	    {
	        $qtyall=$assign_stockist_all-$remove_stockist;
	        $asorqty=$remove_stockist;
	    }
	    else
	    {
	        $qtyall=$assign_stockist+$assign_stockist_all;
	        $asorqty=$assign_stockist;
	    }
	    // update stock
	    if($qtyall>0)
	    {
	        $this->db->update('eshop_stock',array('assign_stockist'=>$qtyall),array('product_id'=>$id));
	    }
	     $countstockist=$this->db->query("SELECT * from eshop_stock_stockist where product_id='".$id."' and stockist_id='".$stockist_id."'")->num_rows();
	     if($countstockist)
	     {
	         $resstock=$this->db->query("SELECT * from eshop_stock_stockist where product_id='".$id."' and stockist_id='".$stockist_id."'")->row();
	         if($type=='remove_stockist_')
	         {
	             $qtystock=$resstock->qty-$remove_stockist;
	             $t=0;
	             
	         }
	         else
	         {
	            $qtystock=$resstock->qty+$assign_stockist;
	            $t=1;
	            
	         }
	         if($qtystock>0)
	         {
	            $this->db->update('eshop_stock_stockist',array('qty'=>$qtystock),array('product_id'=>$id,'stockist_id'=>$stockist_id));
	            $this->db->insert('eshop_stock_stockist_history',array('qty'=>$asorqty,'product_id'=>$id,'stockist_id'=>$stockist_id,'type'=>$t));
	         }
	         if($type=='remove_stockist_')
	         {
	             $status="success";
	             $msg="Quantity Removed Successfully";
	         }
	         else
	         {
	         $status="success";
	     ...
	     else
	     {
	         ...
	         {
	             $status="error";
	             $msg="Quantity can not removed";
	         }
	         else
	         {
	         $this->db->insert('eshop_stock_stockist',array('qty'=>$asorqty,'product_id'=>$id,'stockist_id'=>$stockist_id));
	         $this->db->insert('eshop_stock_stockist_history',array('qty'=>$asorqty,'product_id'=>$id,'stockist_id'=>$stockist_id));
	         
	         $status="success";
	         $msg="Quantity Inserted Successfully";
	         }
	    ...
	    $resstock=$this->db->query("SELECT * from eshop_stock_stockist where product_id='".$id."' and stockist_id='".$stockist_id."'")->row();
	    $newstockistqty=$resstock->qty;
	    echo json_encode(array('status'=>$status,'msg'=>$msg,'type'=>$type,'qty'=>$qty,'remove_stockist'=>$remove_stockist,'assign_stockist'=>$assign_stockist,'assign_stockist_old'=>$newstockistqty));
