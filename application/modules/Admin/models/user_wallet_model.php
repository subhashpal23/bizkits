...
class User_Wallet_Model extends Common_Model
{
      ...
      public function getAllUserWalletBalance()
      {
      	...
      	$result=$this->db->select(array('u.user_id', 'u.username', 'u.registration_date', 'u.rank_name', 'u.active_status', 'w.amount as balance', 'p.amount as pbalance'))
      	->from('user_registration as u')
      	->join('final_e_wallet as w', 'w.user_id=u.user_id')
      	->join('final_product_wallet as p', 'p.user_id=u.user_id')
      	->get()
      	->result();
      	//pr($result);
      	return $result;
