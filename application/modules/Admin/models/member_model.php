  ...
  public function getAllMembers($params = array(),$type=false)
    {
		...
		*/
		...
		else
		{
		    ...
		    if($params['user_id']!='')
		    {
		        $query=$this->db->select('*')->from('user_registration')->where('member_type', 2)->where('user_id',$params['user_id'])->get()->result();
		        $totalData = $totalFiltered = $this->db->select('*')->from('user_registration')->where('member_type', 2)->where('user_id',$params['user_id'])->get()->num_rows();
		    }
		    else if($params['username']!='')
		    {
		        $query=$this->db->select('*')->from('user_registration')->where('member_type', 2)->where('username',$params['username'])->get()->result();
		        $totalData = $totalFiltered = $this->db->select('*')->from('user_registration')->where('member_type', 2)->where('user_id',$params['user_id'])->get()->num_rows();
		    }
		    else
		    {
		        $query=$this->db->select('*')->from('user_registration')->where('member_type', 2)->get()->result();
		        $totalData = $totalFiltered = $this->db->select('*')->from('user_registration')->where('member_type', 2)->get()->num_rows();
		    }
            ...
            if(array_key_exists("start",$params) && array_key_exists("limit",$params)){ 
                    ...
                    if($params['user_id']!='')
        		    {
        		        $query=$this->db->select('*')->from('user_registration')->where('member_type', 2)->where('user_id',$params['user_id'])->limit($params['limit'],$params['start'])->get()->result();
        		        $totalData = $totalFiltered = $this->db->select('*')->from('user_registration')->where('member_type', 2)->where('user_id',$params['user_id'])->limit($params['limit'],$params['start'])->get()->num_rows();
        		    }
        		    else if($params['username']!='')
        		    {
        		        $query=$this->db->select('*')->from('user_registration')->where('member_type', 2)->where('username',$params['username'])->limit($params['limit'],$params['start'])->get()->result();
        		        $totalData = $totalFiltered = $this->db->select('*')->from('user_registration')->where('member_type', 2)->where('user_id',$params['user_id'])->limit($params['limit'],$params['start'])->get()->num_rows();
        		    }
        		    else
        		    {
        		        $query=$this->db->select('*')->from('user_registration')->where('member_type', 2)->limit($params['limit'],$params['start'])->get()->result(); 
                        $totalData = $totalFiltered = $this->db->select('*')->from('user_registration')->where('member_type', 2)->limit($params['limit'],$params['start'])->get()->num_rows();
        		    }
                }elseif(!array_key_exists("start",$params) && array_key_exists("limit",$params)){ 
                    
                    if($params['user_id']!='')
        		    {
        		        $query=$this->db->select('*')->from('user_registration')->where('member_type', 2)->where('user_id',$params['user_id'])->limit($params['limit'])->get()->result();
        		        $totalData = $totalFiltered = $this->db->select('*')->from('user_registration')->where('member_type', 2)->where('user_id',$params['user_id'])->limit($params['limit'])->get()->num_rows();
        		    }
        		    else if($params['username']!='')
        		    {
        		        $query=$this->db->select('*')->from('user_registration')->where('member_type', 2)->where('username',$params['username'])->limit($params['limit'])->get()->result();
        		        $totalData = $totalFiltered = $this->db->select('*')->from('user_registration')->where('member_type', 2)->where('user_id',$params['user_id'])->limit($params['limit'])->get()->num_rows();
        		    }
        		    else
        		    {
        		        $query=$this->db->select('*')->from('user_registration')->where('member_type', 2)->limit($params['limit'])->get()->result(); 
                        $totalData = $totalFiltered = $this->db->select('*')->from('user_registration')->where('member_type', 2)->limit($params['limit'])->get()->num_rows();
        		    }
                } 
   ...
   public function getAllMembersPassword($params = array(),$type=false)
    {
		...
		else
		{
            ...
            foreach ($query as $row) 
			{
				...
				$reset_password=html_entity_decode('<a href="'.ci_site_url().'Admin/Member/resetPassword/'.ID_encode($row->user_id).'"><i class="fa fa-key"></i></a>');
				$reset_password1=html_entity_decode('<a href="'.ci_site_url().'Admin/Member/resetTPassword/'.ID_encode($row->user_id).'">Reset Transaction Password</a>');
   ...
   public function getAllBlockUnBlockMembers($params=array())
    {
		...
		if(array_key_exists("returnType",$params) && $params['returnType'] == 'count'){ 
		    ...
		    $sql2=$this->db->select('u.id,u.username,u.user_id,u.ref_id,u.registration_date,u.active_status,u.member_type,u.pkg_id,u.pkg_amount,u.idno,u.first_name,u.last_name,u.contact_no')
			->from('user_registration as u')->where('u.member_type', 2); 
		    if($params['user_id']!='')
		    {
		        $sql2=$sql2->where('user_id',$params['user_id']);
		    }
		...
		if (!empty($requestData['search']['value'])) {
            ...
            foreach ($query as $row) 
			{
				...
				$action=html_entity_decode(' <a onclick="return changeStatusConfirm('.$row->active_status.');" href="'.ci_site_url().'Admin/Member/changeStatus/'.ID_encode($row->user_id).'" data-popup="tooltip" title="" data-original-title="'.$status_change_tooltip.'"><i class="'.$status_change_icon.'"></i></a>
                           ');
