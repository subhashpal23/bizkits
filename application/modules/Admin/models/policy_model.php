...
class Policy_Model extends Common_Model
{
    ...
    {
 		$result=$this->db->select('*')->from('confidential')->where('confidential_key','notice')->get()->row();
 		return $result;
    }
    public function getNotice1()
    {
 		$result=$this->db->select('*')->from('confidential')->where('confidential_key','notice1')->get()->row();
 		return $result;
    }
