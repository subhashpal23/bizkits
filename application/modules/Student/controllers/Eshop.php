...
class Eshop extends Common_Controller 
{
	...
	public function ewalletPaymentConfirm()
	{
		...
		if(!empty($this->input->post('ewallet_payment_btn')))
		{
				...
				foreach($cart as $product)
				{
					...
					/*$commission=($cart_final_price*4)/100;
					// self commission
					$this->db->insert('eshop_bonus',array(
                        	...
                        	$this->db->insert('credit_debit',array(
                        	    ...
                        	    'order_id'=>$order_id,
                        	    'ttype'=>'Eshop Income',
                        	    'tran_description'=>currency().$commission.' Commission by '.$user_id,
                        	    'status'=>'1',
                        	    'payment_method'=>'1',
					...
					/*$nom_info=$this->db->select('ref_id')->from('user_registration')->where(array('user_id'=>$user_id))->get()->row();
                        $ref_id=$nom_info->ref_id;
                        $commission_ref=($cart_final_price*0.5)/100;
    				    if($ref_id!='' && $ref_id!='cmp')
    				    {
                        	...
                        	$this->db->insert('credit_debit',array(
                        	    ...
                        	    'receive_date'=>date('d-m-Y'),
                        	    'ttype'=>'Eshop Income',
                        	    'tran_description'=>currency().$commission_ref.' Commission by '.$user_id,
                        	    'status'=>'1',
                        	    'payment_method'=>'1',
                        	    'payment_method_name'=>'E-wallet',
                        	    'current_url'=>site_url(),
	...
	{
	    $array=array('5','3','2','2','1','1');
	    $commissionself=($pv*5)/100;
	    ...
	    $query=$this->db->select('*')->from('level_income_binary')->where('down_id',$user_id)->get()->result();
