...
@package user/epin
*/
class Fees extends Common_Controller 
{
	private $userId;
	...
	*/
	...
	{
	    $random_number="Fees".mt_rand(100000, 999999);
	    if($this->db->select('order_id')->from('student_fees')->where('order_id',$random_number)->get()->num_rows()>0)
	    {
	...
	*/
	...
	{
	    $data['all_fees']=$this->db->select('*')->from('student_fees')->where('student_id',$this->userId)->get()->result();
		_userLayout("epin-mgmt/fees-list",$data);
	}
	public function makePayment()
	{
	    if(!empty($this->input->post('btn')))
		{
		   ...
		   $date = str_replace('/', '-', $fee_date);
           $fee_date1=date('Y-m-d', strtotime($date));
		   $this->db->insert('student_fees',array(
			...
			$this->session->set_flashdata("flash_msg",'<span class="text-semibold">Well done!</span>Fees paid successfully!');	
			redirect(ci_site_url()."Student/Fees/feeList");
		}
	    _userLayout("epin-mgmt/make-payment");
	}
}//end class
