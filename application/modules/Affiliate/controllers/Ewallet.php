...
class Ewallet extends Common_Controller 
{
	...
	*/
	public function viewEwalletBalance()
	{
		...
		$data['twallet_balance']=$this->ewallet_model->getTwalletBalance($this->userId);
	...
	*/
	public function check_valid_username($username)
	{
		...
		else 
		{
			...
			$query=$this->db->select('user_id')->from('user_registration')->get();
			if($query->num_rows()<1)
			{
	...
	*/
	public function check_valid_tran_password($password)
	{
		...
		{
			$query=$this->db->query("SELECT user_id FROM (`user_registration`) WHERE (`username` = '$username' OR `user_id` = '$username') AND `t_code` = '$password'");
			if($query->num_rows()<1)
			{
    ...
    if(!empty($this->input->post('btn')))
    {
        ...
        if(is_numeric($deposit_amount))
        {
            if($deposit_amount>0)
            {
                ...
                redirect(ci_site_url()."Affiliate/Ewallet/purchaseFund");
            }
            else
            {
                $this->session->set_flashdata("error_msg",'<span class="text-semibold">Oops!</span> Entered amount should be greater than zero!');
                redirect(ci_site_url()."Affiliate/Ewallet/purchaseFund");
            }
        }
        else
        {
            $this->session->set_flashdata("error_msg",'<span class="text-semibold">Oops!</span> Please enter valid amount!');
            redirect(ci_site_url()."Affiliate/Ewallet/purchaseFund");
        }
     }//end submit if here
	...
	*/
	public function ewalletFundTransfer()
	{
		...
		if(!empty($this->input->post('btn')))
		{
    		 ...
    		 // check username and transaction password
    		 $count=$this->db->select('user_id')->from('user_login')->where(array('username'=>$username))->get()->num_rows();
    		 if($count>0)
    		 {
    		     $count_pass=$this->db->select('user_id')->from('user_login')->where(array('user_id'=>$this->userId,'t_code'=>$tran_password))->get()->num_rows();
    		     // check 
    		    if($count_pass>0)
    		    {
    		        $to_user=$this->member_model->getUserId($username);
    		        if($this->userId==$to_user)
    		        {
    		            $this->session->set_flashdata("error_msg",'<span class="text-semibold">Oops!</span>Can not send to self!');
                	        redirect(ci_site_url()."Affiliate/Ewallet/ewalletFundTransfer");
                			 exit;
    		        }
    		        if(is_numeric($amount))
                    {
                         if($amount>0)
                         {
                    		 ...
                    		 /*if(!$this->form_validation->run()==false)
                		  	...
                		  	$user=$this->db->select('amount')->from('final_product_wallet')->where('user_id',$this->userId)->get()->row();
                		  	if($user->amount>=$amount)
                		  	{
                        		  	$final_balance=$user->amount-$amount;
                        		  	$this->db->update('final_product_wallet',array('amount'=>$final_balance),array('user_id'=>$this->userId));
                        			...
                        			$this->db->insert('credit_debit_product',array(
                        		  	...
                        		  	$user=$this->db->select('amount')->from('final_product_wallet')->where('user_id',$to_user)->get()->row();
                        		  	$final_balance=$user->amount+$amount;
                        		  	$this->db->update('final_product_wallet',array('amount'=>$final_balance),array('user_id'=>$to_user));
                        			//@credit entry
                        			$this->db->insert('credit_debit_product',array(
                        					'transaction_no'=>generateUniqueTranNo(),
                        					'user_id'=>$to_user,
                		  	...
                		  	{
                		  	    $this->session->set_flashdata("error_msg",'<span class="text-semibold">Oops!</span>No sufficient fund in wallet!');
                	        redirect(ci_site_url()."Affiliate/Ewallet/ewalletFundTransfer");
                			 exit;
                		  	}
                         }
                         ...
                         {
                             $this->session->set_flashdata("error_msg",'<span class="text-semibold">Oops!</span>Amount should be greater than zero!');
                	        redirect(ci_site_url()."Affiliate/Ewallet/ewalletFundTransfer");
                			 exit;
                         }
    		        }
    		     ...
    		     {
    		         $this->session->set_flashdata("error_msg",'<span class="text-semibold">Oops!</span>Wrong transaction password!');
        	          redirect(ci_site_url()."Affiliate/Ewallet/ewalletFundTransfer");
        			  exit;
    		     }
    		 }
    		 ...
    		 {
    		      $this->session->set_flashdata("error_msg",'<span class="text-semibold">Oops!</span>Wrong receiver username!');
    	          redirect(ci_site_url()."Affiliate/Ewallet/ewalletFundTransfer");
    			  exit;
    		 }
		}
	...
	*/
	...
	{
		$data=array();
		$data['title']='Bonus Wallet Statements';
	...
	public function viewTwalletStatement()
	{
		$data=array();
		$data['title']='Transaction Wallet Statements';
        $all_statements=$this->ewallet_model->getTwalletStatements($this->userId);
		$data['all_statements']=$all_statements;
		$data['fundtshow']='show';
		_userLayout("ewallet-mgmt/view-ewallet-statement",$data);
	}//end method
      ...
      if(!empty($this->input->post('btn')))
       {
        ...
        redirect(ci_site_url()."Affiliate/Ewallet/depositWalletAmountRequestList");
       }//end submit if here
