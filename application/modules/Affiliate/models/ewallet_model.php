...
class Ewallet_Model extends Common_Model
{
  ...
  public function getEwalletStatements($user_id)
   ...
   public function getTwalletBalance($user_id)
  {
     $res=$this->db->select('amount')->from('final_product_wallet')->where('user_id', $user_id)->get()->row();
     $balance=(!empty($res->amount))?$res->amount:0;
     return $balance;   
  }//end method
  public function getTwalletStatements($user_id)
   {
      $statementQuery=$this->db->select('cd.transaction_no,cd.status, cd.user_id, cd.sender_id, cd.receiver_id, cd.credit_amt, cd.debit_amt, cd.balance, cd.ttype as title, cd.TranDescription as description, cd.create_date as date')->from('credit_debit_product as cd')
      ->join('user_registration as u', 'u.user_id=cd.user_id')
      ->order_by('cd.id','DESC')
      ->where('u.user_id',$user_id)
      ->get();
