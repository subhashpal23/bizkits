...
class Dashboard_Model extends Common_Model
{
   ...
   */
   ...
   public function getUserTotalDirectCommissionWeekly($user_id)
   {
     $credit_amount_info=$this->db->select_sum('credit_amt')->from('credit_debit')->where(array('user_id'=>$user_id,'reason'=>'5','receive_date >='=>date('Y-m-d',strtotime("last Monday"))))->get()->row();
     $credit_amount=(!empty($credit_amount_info->credit_amt))?$credit_amount_info->credit_amt:'0';
	 return $credit_amount+$credit_amount1;
   }
   public function getUserTotalLevelCommissionWeekly($user_id)
   {
     $credit_amount_info=$this->db->select_sum('credit_amt')->from('credit_debit')->where(array('user_id'=>$user_id,'reason'=>'9','receive_date >='=>date('Y-m-d',strtotime("last Monday"))))->where_in('reason',array('9'))->get()->row();
     $credit_amount=(!empty($credit_amount_info->credit_amt))?$credit_amount_info->credit_amt:'0';
	 return $credit_amount+$credit_amount1;
   }
   public function getUserTotalBinaryCommissionWeekly($user_id)
	{
		 $credit_amount_info=$this->db->select_sum('credit_amt')->from('credit_debit')->where(array('user_id'=>$user_id,'reason'=>'6','receive_date >='=>date('Y-m-d',strtotime("last Monday"))))->get()->row();
		 $credit_amount=(!empty($credit_amount_info->credit_amt))?$credit_amount_info->credit_amt:'0';
		 return $credit_amount+$credit_amount1;
	}
   public function getUserTotalFastStartCommissionWeekly($user_id)
   {
     $credit_amount_info=$this->db->select_sum('credit_amt')->from('credit_debit')->where(array('user_id'=>$user_id,'reason'=>'8','receive_date >='=>date('Y-m-d',strtotime("last Monday"))))->where_in('reason',array('8'))->get()->row();
     $credit_amount=(!empty($credit_amount_info->credit_amt))?$credit_amount_info->credit_amt:'0';
     return $credit_amount+$credit_amount1;
   }
   ...
   public function getSelfCommission($user_id)
    {
        $credit_amount_info=$this->db->select_sum('credit_amt')->from('credit_debit_cash')->where(array('user_id'=>$user_id,'sender_id'=>$user_id,'reason'=>'10'))->where_in('reason',array('10'))->get()->row();
    ...
    {
         $credit_amount_info=$this->db->select_sum('credit_amt')->from('credit_debit_reward')->where(array('user_id'=>$user_id,'reason'=>'13'))->where_in('reason',array('13'))->get()->row();
    $credit_amount=(!empty($credit_amount_info->credit_amt))?$credit_amount_info->credit_amt:'0';
     return $credit_amount+$credit_amount1;
    }
    public function getStockistCommission($user_id)
    {
         $credit_amount_info=$this->db->select_sum('credit_amt')->from('credit_debit')->where(array('user_id'=>$user_id,'reason'=>'18'))->where_in('reason',array('18'))->get()->row();
    $credit_amount=(!empty($credit_amount_info->credit_amt))?$credit_amount_info->credit_amt:'0';
     return $credit_amount+$credit_amount1;
    }
   public function getUserTotalFastStartCommission($user_id)
   {
     $credit_amount_info=$this->db->select_sum('credit_amt')->from('credit_debit')->where(array('user_id'=>$user_id,'reason'=>'8'))->where_in('reason',array('8'))->get()->row();
