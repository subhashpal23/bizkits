...
class Auth_Model extends Common_Model
{
  ...
  */
  ...
  {
      $where_condition="(username = '$username' OR email = '$username') and password='$password'";
      ...
      // echo $query1->num_rows(); die;
    	if($query1->num_rows() > 0)
    	{
    	    $where_condition="(username = '$username' OR email = '$username') and password='$password'";
					...
					$res=$query->row();  
            // pr($res->member_type  ); exit;
        	$this->userName=$res->username;
        	$this->userPassword=$res->password;
        	$this->user_id=$res->user_id;
            $this->SD_User_Name=$res->username;
            $this->userpanel_user_id=$res->user_id;
            $this->member_type=$res->member_type;
            $guest_ip   = $_SERVER['REMOTE_ADDR'];
            $fullName=$res->first_name." ".$res->last_name;
            $this->db->insert("visitor",array('user_id'=>$res->user_id,'username'=>$res->username,'fullname'=>$fullName,'ipadd'=>$guest_ip));
        	return true;
    	}
      else 
    	return false;
