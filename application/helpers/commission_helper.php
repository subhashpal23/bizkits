...
function getBinaryPairingCommission($user_id=null,$pkg_id=null,$date=null)
{
   $obj=& get_instance();
   if($date=='')
   {
	...
	/////////////
     ...
     //echo $leftamt.'=='.$rightamt.'=='.$pairpv; exit;
	 if((!empty($leftamt) && !empty($rightamt)) && (($leftamt>=$pairpv) && ($rightamt>=$pairpv)))
	 {
			...
			$capping_amount=$pairpv*$capping_times;
			...
			// condition here
			// if current pv and total bonus is less than capping pv then process pv= current pv
			// else if current pv and total bonus is equal to capping pv then process pv = current pv
			// else if current pv and total bonus is greater than capping pv then process pv = (capping pv-total bonus)~current pv , carry pos= current pv pos ; carry forword=(current pv+total bonus)-capping pv  carry pos= current pv pos 
			
			$totalpv=$totaldailybonus+$lesser_bv;
			
			if($totalpv<$capping_amount)
			{
			    $commission_amount=$lesser_bv;
			    
			    if($carry_pos=='Left')
    			    {
    			        $carry_amount_right=0;
    			        $carry_amount_left=($bigger_bv+$totaldailybonus)-$lesser_bv;
    			    }
    			    else if($carry_pos=='Right')
    			    {
    			        $carry_amount_left=0;
    			        $carry_amount_right=($bigger_bv+$totaldailybonus)-$lesser_bv;
    			    }
    			    else
    			    {
    			        $carry_amount_left=0;
			...
			else if($totalpv==$capping_amount)
			{
			    $commission_amount=$lesser_bv;
			    if($carry_pos=='Left')
    			    {
			...
			else if($totaldailybonus>=$capping_amount)
			{
			    ...
			    if($carry_pos=='Left')
    			    {
    			        $carry_amount_right=0;
    			        $carry_amount_left=($bigger_bv+$totaldailybonus)-0;
    			    }
    			    else if($carry_pos=='Right')
    			    {
    			        $carry_amount_left=0;
    			        $carry_amount_right=($bigger_bv+$totaldailybonus)-0;
    			    }
    			    else
    			    {
    			        $carry_amount_left=0;
			...
			{
			    $diff=$capping_amount-$totaldailybonus;
			    $commission_amount=$diff;
			     /*if ($diff > $lesser_bv) {
                    $commission_amount=$diff - $lesser_bv;
                  } else {
                    $commission_amount=$lesser_bv - $diff;
                  }*/
                  ...
                  if($carry_pos=='Left')
    			    {
    			        $carry_amount_right=0;
    			        $carry_amount_left=($bigger_bv+$totaldailybonus)-$capping_amount;
    			    }
    			    else if($carry_pos=='Right')
    			    {
    			        $carry_amount_left=0;
    			        $carry_amount_right=($bigger_bv+$totaldailybonus)-$capping_amount;
    			    }
    			    else
    			    {
    			        $carry_amount_left=0;
	 ...
	 'user_id'=>$user_id,
	 'matching_pv'=>$matchingpv,
	 'daily_binary_cycle'=>$daily_binary_cycle
	 );
        	    ...
        	    //echo "syubhahsj";
        			foreach($all_user_query->result() as $userObj)
        			{
        					...
        					if($pkg_id>1)
        					{
            					...
            					//echo "Hello";exit;
            					$commission_details=getBinaryPairingCommission($user_id,$pkg_id,$date);
            					//echo $user_id;
            					//pr($commission_details);
            					//echo $matchingcondleft.'=='.$matchingcondright; //exit;
            					//exit;
            					if(!empty($commission_details->commission_amount) && $matchingcondleft>=1 && $matchingcondright>=1)
            					{
            					    $matched_pv=$commission_details->commission_amount;
            					    $obj->db->insert('matching_bonus',array(
            								'user_id'=>$user_id,
            								'bonus'=>$commission_details->lessar_pv,
            								'bonus_date'=>$date,
            								'bonus_details'=>json_encode($commission_details),
            								'matched_pv'=>$matched_pv,
            								'status'=>'0'
            								));
            								
            						$matching_id=$obj->db->insert_id();	
...
function creditBinaryLevelCommissionAuto($down_id,$date,$current_timestamp)
{
	//insert commission in user ewallet by fetching from level income table code start here
	$obj=& get_instance();
	$arraycom=array('',30,15,15,10,5);
    	    ...
    	    $userLevelObj->income_id==$down_id;
        	$all_user_query=$obj->db->select('*')->from('user_registration')->where(array('user_id' => $down_id,'nom_id !=' => '','active_status'=>'1'))->get();
